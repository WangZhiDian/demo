			...
			for (Entry<String, Object> entry : map.entrySet()) {
				if (SPECIAL_ENTITIY.contains(entry.getKey())) {// 判断是否在之中
					...
					if (ENTITY_STRING.equals(entry.getKey())) {// string
						...
						break;
					} else if (ENTITY_BYTES.equals(entry.getKey())) {// bytes
						...
						entity = new SerializableEntity((Serializable) entry.getValue());
						break;
						...
						for (File file : files) {
							...
							// builder.addPart(String.valueOf(map.get(ENTITY_MULTIPART+".name"))
							// + count++, fileBody);
