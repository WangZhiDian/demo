package cn.model.maven.commons.httpclient.builder;

import cn.model.maven.commons.httpclient.common.SSLs;
import cn.model.maven.commons.httpclient.common.SSLs.SSLProtocolVersion;
...
public class HCB extends HttpClientBuilder {
	...
	private SSLProtocolVersion sslpv = SSLProtocolVersion.TLSv1;// ssl 协议版本
	...
	public HCB timeout(int conn_timeout, int read_timeout)
	{
		// 配置请求的超时设置
		RequestConfig config = RequestConfig.custom().setConnectionRequestTimeout(conn_timeout).setConnectTimeout(conn_timeout).
	...
	public HCB pool(int maxTotal, int defaultMaxPerRoute) throws HttpProcessException
	{
		Registry<ConnectionSocketFactory> socketFactoryRegistry = RegistryBuilder.<ConnectionSocketFactory> create().
				register("http", PlainConnectionSocketFactory.INSTANCE).register("https", ssls.getSSLCONNSF(sslpv)).build();
		// 设置连接池大小
