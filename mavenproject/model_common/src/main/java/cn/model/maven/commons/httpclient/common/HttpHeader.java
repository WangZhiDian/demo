package cn.model.maven.commons.httpclient.common;
...
public class HttpHeader {
	...
	/**
	 * 表示是否需要持久连接（HTTP 1.1默认进行持久连接） 例如：Connection: close 短链接； Connection:
	 * keep-alive 长连接
	 * 
	 * @param connection
	 * @return
	...
	/**
	 * HTTP请求发送时，会把保存在该请求域名下的所有cookie值一起发送给web服务器 例如：Cookie: $Version=1;
	 * Skin=new;
	 * 
	 * @param cookie
	 */
	...
	/**
	 * 如果请求的部分在指定时间之后被修改则请求成功，未被修改则返回304代码 例如：If-Modified-Since: Sat, 29 Oct
	 * 2010 19:43:31 GMT
	 * 
	...
	/**
	 * 如果实体未改变，服务器发送客户端丢失的部分，否则发送整个实体。参数也为Etag 例如：If-Range:
	 * “737060cd8c284d8af7ad3082f209582d”
	 * 
	 * @param ifRange
	 * @return
	...
	/**
	 * 只在实体在指定时间之后未被修改才请求成功 例如：If-Unmodified-Since: Sat, 29 Oct 2010 19:43:31
	 * GMT
	 * 
	...
	/**
	 * 向服务器指定某种传输协议以便服务器进行转换（如果支持） 例如：Upgrade: HTTP/2.0, SHTTP/1.3, IRC/6.9,
	 * RTA/x11
	 * 
	 * @param upgrade
	 */
